...
void main(void)
{
    ...
    PCA_SetClockSource(PCA_ClockSource_SysClkDiv12);
    ...
    PCA_PWM0_SetBitWidth(PCA_PWM_BitWidth_10);
    PCA_PWM1_SetBitWidth(PCA_PWM_BitWidth_10);
    // Set initial duty cycle
    PCA_PCA0_SetCompareValue10bit(dc);
    PCA_PCA1_SetCompareValue10bit(~dc & 0x3FF);
    ...
    while(1)
    {
        PCA_PCA0_ChangeCompareValue10bit(dc);
        PCA_PCA1_ChangeCompareValue10bit(~dc & 0x3FF);
        UART1_TxHex(dc >> 8);
        UART1_TxHex(dc & 0xFF);
        if (dir)
        {
            dc++;
            if (dc == 0x03FF) dir = !dir;
        }
        else
        {
            dc--;
